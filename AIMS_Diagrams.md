...
sequenceDiagram
    actor User
    participant Header as :Header<<boundary>>
    participant LogoutModal as :LogoutModal<<boundary>>
    participant AuthenticationController as :AuthenticationController<<controller>>
    participant AuthenticationService as :AuthenticationService<<controller>>
    participant InvalidatedTokenRepository as :InvalidatedTokenRepository<<controller>>
    participant InvalidatedToken as :InvalidatedToken<<entity>>

    User->>Header: 1. clickUserMenu()
    Header-->>User: 2. showDropdownMenu()
    
    User->>Header: 3. clickLogout()
    Header->>LogoutModal: 4. showLogoutConfirmation()
    LogoutModal-->>User: 5. displayConfirmDialog("Are you sure you want to log out?")
    
    alt userCancels
        User->>LogoutModal: 6a. clickCancel()
        LogoutModal-->>Header: 7a. cancelled
        Header-->>User: 8a. closeModal()
    else userConfirms
        User->>LogoutModal: 6b. clickConfirm()
        LogoutModal->>LogoutModal: 7b. getTokenFromStorage()
        
        LogoutModal->>AuthenticationController: 8b. POST /auth/logout (LogoutRequest)
        AuthenticationController->>AuthenticationService: 9. logout(request)
        
        AuthenticationService->>AuthenticationService: 10. verifyToken(token)
        
        alt tokenInvalid
            AuthenticationService-->>AuthenticationController: 11a. throw WebException(UNAUTHENTICATED)
            AuthenticationController-->>LogoutModal: 12a. ApiResponse(error)
            Note over LogoutModal: Vẫn xóa token local dù server báo lỗi
        else tokenValid
            AuthenticationService->>AuthenticationService: 11b. parseJWT(token)
            AuthenticationService->>AuthenticationService: 12b. extractJWTID(signedJWT)
            AuthenticationService->>AuthenticationService: 13b. extractExpiryTime(signedJWT)
            
            AuthenticationService->>InvalidatedToken: 14b. create(jwtId, expiryTime)
            InvalidatedToken-->>AuthenticationService: invalidatedToken
            
            AuthenticationService->>InvalidatedTokenRepository: 15b. save(invalidatedToken)
            InvalidatedTokenRepository-->>AuthenticationService: saved
            
            AuthenticationService-->>AuthenticationController: 16b. void
            AuthenticationController-->>LogoutModal: 17b. ApiResponse<Void>
        end
        
        LogoutModal->>LogoutModal: 18. clearTokenFromStorage()
        LogoutModal->>LogoutModal: 19. clearUserSession()
        LogoutModal-->>User: 20. redirectToLoginPage()
    end
```
...
classDiagram
    ...
    class AuthenticationService {
        <<controller>>
        +logout(request)
        -verifyToken(token): SignedJWT
        -parseJWT(token): SignedJWT
    }
    class InvalidatedTokenRepository {
        <<controller>>
        +save(token): InvalidatedToken
    ...
    Header --> LogoutModal
    LogoutModal --> AuthenticationController
    AuthenticationController --> AuthenticationService
    AuthenticationService --> InvalidatedTokenRepository
    InvalidatedTokenRepository --> InvalidatedToken
```

### 12.3 Communication Diagram
...
flowchart LR
    ...
    User -->|"1: clickUserMenu()"| Header
    User -->|"2: clickLogout()"| Header
    Header -->|"3: showLogoutConfirmation()"| LogoutModal
    User -->|"4: clickConfirm()"| LogoutModal
    LogoutModal -->|"5: POST /auth/logout"| AuthenticationController
    AuthenticationController -->|"6: logout()"| AuthenticationService
    AuthenticationService -->|"7: verifyToken()"| AuthenticationService
    AuthenticationService -->|"8: create()"| InvalidatedToken
    AuthenticationService -->|"9: save()"| InvalidatedTokenRepository
    LogoutModal -->|"10: clearTokenFromStorage()"| LogoutModal
```
...
Tài liệu này bao gồm các diagram cho 12 use case của hệ thống AIMS:

| Use Case | Mô tả | Actors |
|----------|-------|--------|
| UC1 | Place Order | Customer |
| UC2 | Add Product to Cart | Customer |
| UC3 | Pay Order (VietQR) | Customer |
| UC4 | Pay Order by Credit Card | Customer |
| UC5 | Select Delivery Method | Customer |
| UC6 | View Product Details | Customer |
| UC7 | Search Products | Customer |
| UC8 | Create Product | Product Manager |
| UC9 | Update Product | Product Manager |
| UC10 | Delete Product | Product Manager |
| UC11 | Log In | User (Customer/Admin/PM) |
| UC12 | Log Out | User (Customer/Admin/PM) |

### Kiến trúc hệ thống

Hệ thống AIMS sử dụng kiến trúc 3-layer:
- **Boundary (Presentation Layer)**: React Components (Frontend)
- **Controller (Business Logic Layer)**: Spring Boot Controllers & Services (Backend)
- **Entity (Data Layer)**: JPA Entities (Database)
